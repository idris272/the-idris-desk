...
export const AuthAPI = {
  ...
  async login({ usernameOrEmail, password }) {
    ...
    return stampToISO({ uid: cred.user.uid, ...userSnap.data() });
  },
...
export const PostsAPI = {
  ...
  // Create a new post
  async create({ title, content, excerpt, category, tags, coverImage, status, featured, authorUid, authorName }) {
  ...
  async update(postId, updates) {
    const allowed = ["title", "slug", "content", "excerpt", "category", "tags", "coverImage", "status", "featured", "readTime"];
    const safe = Object.fromEntries(
      Object.entries(updates).filter(([k]) => allowed.includes(k))
    );
