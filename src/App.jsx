...
const renderMd = (text) => {
  if (!text) return "";
  return text
    // Images with captions: ![caption text](image-url)
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<figure style="margin:2em 0;text-align:center"><img src="$2" alt="$1" style="max-width:100%;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.1)" /><figcaption style="margin-top:10px;font-size:0.875rem;color:var(--text-tertiary);font-style:italic;line-height:1.5">$1</figcaption></figure>')
    // Headings
    ...
    .replace(/\*(.*?)\*/g, '<em>$1</em>')
    // Blockquotes: > text
    .replace(/^> (.*$)/gm, '<blockquote style="border-left:4px solid var(--accent);padding:12px 20px;margin:1.5em 0;background:var(--accent-light);border-radius:0 8px 8px 0;font-style:italic;color:var(--text-secondary)">$1</blockquote>')
    // Horizontal rules: ---
    .replace(/^---$/gm, '<hr style="border:none;border-top:2px solid var(--border);margin:2em 0" />')
    // Lists
    .replace(/^- (.*$)/gm, '<li>$1</li>')
    // Paragraphs
    .replace(/\n\n/g, '</p><p>')
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    // Safety net: if posts haven't loaded after 4 seconds (e.g. onSnapshot
    // failed silently for a guest), do a direct fetch
    const fallbackTimer = setTimeout(() => {
      if (posts.length === 0) {
        PostsAPI.getPublished().then(fetched => {
...
// CommentItem: single comment with like/dislike + reply thread
const CommentItem = ({ comment, currentUser, articleId, addToast }) => {
  const [reactions,    setReactions]    = useState({ likes: comment.likes || 0, dislikes: comment.dislikes || 0, mine: null });
  ...
  // Load my existing reaction for this comment
  useEffect(() => {
    if (!currentUser?.uid) return;
    CommentsAPI.getReactions([comment.id], currentUser.uid).then(r => {
      if (r[comment.id]) setReactions(prev => ({ ...prev, mine: r[comment.id] }));
    }).catch(() => {});
  }, [comment.id, currentUser]);

  const handleReact = async (reaction) => {
    if (!currentUser && !replyName) {
      // Allow guests to react too — just toggle optimistically since we can't track them
    }
    const uid = currentUser?.uid || ("guest_" + Math.random().toString(36).slice(2));
    const prev = reactions.mine;
    // Optimistic update
    setReactions(r => {
      const n = { ...r };
      if (prev === reaction) {
        n[reaction === "like" ? "likes" : "dislikes"]--;
        n.mine = null;
      } else {
        if (prev) n[prev === "like" ? "likes" : "dislikes"]--;
        n[reaction === "like" ? "likes" : "dislikes"]++;
        n.mine = reaction;
      }
      return n;
    ...
    try {
      await CommentsAPI.toggleReaction({ targetType: "comment", targetId: comment.id, uid, reaction });
    } catch {
      // revert optimistic update on failure
      setReactions({ likes: comment.likes || 0, dislikes: comment.dislikes || 0, mine: prev });
    }
  };

  const loadReplies = async () => {
    if (showReplies) { setShowReplies(false); return; }
    const r = await CommentsAPI.getReplies(comment.id);
    setReplies(r);
    setShowReplies(true);
  };

  const submitReply = async () => {
    const name = currentUser?.displayName || replyName.trim();
    if (!replyText.trim() || !name) { addToast("Please enter your name and reply", "info"); return; }
    setLoadingReply(true);
    try {
      const r = await CommentsAPI.reply({
    ...
    } catch { addToast("Failed to post reply", "error"); }
    finally { setLoadingReply(false); }
  };

  const reactBtn = (type, icon, count) => (
    <button onClick={() => handleReact(type)} style={{ display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", border: "1px solid var(--border)", borderRadius: 20, background: reactions.mine === type ? "var(--accent-light)" : "none", color: reactions.mine === type ? "var(--accent)" : "var(--text-tertiary)", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
      {icon} {count}
    </button>
  );

  return (
    <div style={{ padding: 20, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <Avatar name={comment.authorName} size={32} fs={12} />
        <span style={{ fontWeight: 600, fontSize: 14 }}>{comment.authorName}</span>
        {comment.isGuest && <Badge text="Guest" color="#6b7280" />}
        <span style={{ fontSize: 12, color: "var(--text-tertiary)", marginLeft: "auto" }}>{formatDate(comment.createdAt)}</span>
      </div>
      <p style={{ fontSize: 14, lineHeight: 1.6, color: "var(--text-secondary)", marginBottom: 12 }}>{comment.content}</p>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        {reactBtn("like", "👍", reactions.likes)}
        {reactBtn("dislike", "👎", reactions.dislikes)}
        <button onClick={() => setReplyOpen(v => !v)} style={{ display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", border: "1px solid var(--border)", borderRadius: 20, background: "none", color: "var(--text-tertiary)", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
          💬 Reply
        </button>
        <button onClick={loadReplies} style={{ display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", border: "none", background: "none", color: "var(--accent)", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
      ...
      {replyOpen && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--border-light)" }}>
          {!currentUser && <input value={replyName} onChange={e => setReplyName(e.target.value)} placeholder="Your name" style={{ ...inputStyle, marginBottom: 8, maxWidth: 200 }} />}
          <div style={{ display: "flex", gap: 8 }}>
            <Avatar src={currentUser?.avatar} name={currentUser?.displayName || "G"} size={28} fs={11} />
            <div style={{ flex: 1 }}>
              <textarea value={replyText} onChange={e => setReplyText(e.target.value)} placeholder={`Reply to ${comment.authorName}…`} rows={2} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.5, marginBottom: 8 }} />
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                <button onClick={() => setReplyOpen(false)} style={{ padding: "7px 14px", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", background: "none", cursor: "pointer", fontSize: 13, color: "var(--text-secondary)" }}>Cancel</button>
                <button onClick={submitReply} disabled={loadingReply || !replyText.trim()} style={{ padding: "7px 16px", border: "none", borderRadius: "var(--radius-xl)", background: "var(--accent)", color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600, opacity: loadingReply ? 0.7 : 1 }}>{loadingReply ? "Posting…" : "Reply"}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showReplies && replies.length > 0 && (
        <div style={{ marginTop: 14, paddingLeft: 20, borderLeft: "2px solid var(--border-light)", display: "flex", flexDirection: "column", gap: 12 }}>
          {replies.map(r => (
            <div key={r.id} style={{ padding: "12px 16px", background: "var(--bg-secondary)", borderRadius: "var(--radius-md)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <Avatar name={r.authorName} size={24} fs={10} />
                <span style={{ fontWeight: 600, fontSize: 13 }}>{r.authorName}</span>
                {r.isGuest && <Badge text="Guest" color="#6b7280" />}
                <span style={{ fontSize: 11, color: "var(--text-tertiary)", marginLeft: "auto" }}>{formatDate(r.createdAt)}</span>
              </div>
              <p style={{ fontSize: 13, lineHeight: 1.55, color: "var(--text-secondary)" }}>{r.content}</p>
            </div>
          ))}
        </div>
      )}
      {showReplies && replies.length === 0 && (
        <p style={{ fontSize: 13, color: "var(--text-tertiary)", marginTop: 12, paddingLeft: 20 }}>No replies yet.</p>
      )}
    </div>
  );
};

// Fix 5: Share button — uses Web Share API on mobile, fallback menu on desktop
const ShareButton = ({ title, url }) => {
  ...
  const shareOptions = [
    ...
    { label: "X / Twitter", icon: "🐦", href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(url)}` },
    { label: "Telegram", icon: "✈️", href: `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(title)}` },
    { label: "Email", icon: "📧", href: `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(url)}` },
    { label: "Copy Link", icon: "🔗", href: null },
  ];

  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={doShare} style={{ display: "flex", alignItems: "center", gap: 4, padding: "8px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", background: "var(--bg-card)", color: "var(--text-secondary)", cursor: "pointer" }}>
        <I n="share" s={16} />
      </button>
      {showMenu && (
        <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", boxShadow: "var(--shadow-lg)", minWidth: 180, padding: 4, zIndex: 200, animation: "slideDown .2s ease-out" }}>
          {shareOptions.map(opt => (
            opt.href ? (
              <a key={opt.label} href={opt.href} target="_blank" rel="noopener noreferrer"
                style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", fontSize: 14, color: "var(--text-primary)", textDecoration: "none", borderRadius: "var(--radius-sm)", cursor: "pointer" }}
                onMouseEnter={e => e.currentTarget.style.background = "var(--bg-secondary)"}
                onMouseLeave={e => e.currentTarget.style.background = "none"}>
                <span style={{ fontSize: 16 }}>{opt.icon}</span> {opt.label}
              </a>
            ) : (
              <button key={opt.label} onClick={() => { navigator.clipboard?.writeText(url); setShowMenu(false); }}
                style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 14px", fontSize: 14, color: "var(--text-primary)", background: "none", border: "none", borderRadius: "var(--radius-sm)", cursor: "pointer", textAlign: "left" }}
                onMouseEnter={e => e.currentTarget.style.background = "var(--bg-secondary)"}
                onMouseLeave={e => e.currentTarget.style.background = "none"}>
                <span style={{ fontSize: 16 }}>{opt.icon}</span> {opt.label}
              </button>
...
const ArticlePage = ({ articleId }) => {
  ...
  useEffect(() => {
    ...
    const commentFallback = setTimeout(() => {
      CommentsAPI.getForPost(articleId).then(fetched => {
        if (fetched && fetched.length > 0) setComments(prev => prev.length > 0 ? prev : fetched);
      }).catch(() => {});
    }, 3000);
  ...
  // Fix 3: guest comment — wrap in try/catch but CommentsAPI.create itself
  // no longer awaits ActivityAPI.log, so it won't throw for guests
  ...
  return (
    <article className="container" style={{ maxWidth: 820, margin: "0 auto", padding: "40px 24px 80px" }}>
      ...
      <div className="anim-in" style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text-tertiary)" }}>{formatDate(article.publishedAt)} · {article.readTime} min read<LengthBadge length={article.articleLength} /></div>
          </div>
        </div>
      ...
      {/* Fix 4: Enhanced comments with replies, likes, dislikes */}
      <section style={{ borderTop: "1px solid var(--border)", paddingTop: 40 }}>
        ...
        {comments.length === 0
          ...
          : <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {comments.map((c, i) => (
                <div key={c.id} className="anim-in" style={{ animationDelay: `${i * .05}s` }}>
                  <CommentItem comment={c} currentUser={currentUser} articleId={articleId} addToast={addToast} />
                </div>
              ))}
            </div>
        }
  ...
  useEffect(() => {
    ...
    if (editId) {
      PostsAPI.getById(editId).then(p => {
        ...
        setArticleLength(p.articleLength || "standard");
      });
    }
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              ...
              <select value={articleLength} onChange={e => setArticleLength(e.target.value)} style={inputStyle}>
                <option value="short">📄 Short Read (under 5 min)</option>
                <option value="standard">📰 Standard</option>
                <option value="long">📖 Long Read (10+ min)</option>
              </select>
            </div>
          </div>
          <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: "14px 18px", fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.6 }}>
            <strong style={{ color: "var(--text-primary)" }}>📸 Inserting images into your article:</strong><br/>
            Use this format in your content: <code style={{ background: "var(--bg-primary)", padding: "2px 6px", borderRadius: 4, fontFamily: "var(--font-mono)", fontSize: 12 }}>![Caption text](image-url)</code><br/>
            Example: <code style={{ background: "var(--bg-primary)", padding: "2px 6px", borderRadius: 4, fontFamily: "var(--font-mono)", fontSize: 12 }}>![The Golden Gate Bridge at sunset](https://example.com/photo.jpg)</code>
          </div>
...
const LengthBadge = ({ length }) => {
  ...
  const config = {
    short: { text: "📄 Short Read", color: "#06b6d4" },
    long:  { text: "📖 Long Read",  color: "#8b5cf6" },
  };
  const c = config[length];
  if (!c) return null;
...
const RecommendationWidget = ({ currentArticleId = null }) => {
  ...
  const recommended = useMemo(() => {
    ...
    return [...published].sort((a, b) => {
      const scoreA = (a.views || 0) * 0.3 + (a.likes || 0) * 2 + (new Date(a.publishedAt || a.createdAt).getTime() / 1e10);
      const scoreB = (b.views || 0) * 0.3 + (b.likes || 0) * 2 + (new Date(b.publishedAt || b.createdAt).getTime() / 1e10);
  ...
  return (
    <div style={{ position: "fixed", bottom: 24, left: 24, zIndex: 90, maxWidth: 340, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-xl)", animation: "fadeInUp .5s ease-out", overflow: "hidden" }}>
      <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border-light)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--accent-light)" }}>
        ...
        {recommended.map((a, i) => (
          <div key={a.id} onClick={() => { setPage({ name: "article", id: a.id }); setDismissed(true); }}
            ...
            <div>
              ...
              <span style={{ fontSize: 11, color: "var(--text-tertiary)" }}>{a.readTime}m read<LengthBadge length={a.articleLength} /></span>
            </div>
          </div>
        ))}
      </div>
...
const ArticleCard = ({ article, variant = "default", index = 0 }) => {
  ...
  const cat = categories?.find(c => c.id === article.category);
  const lengthBadge = article.articleLength && article.articleLength !== "standard" ? <LengthBadge length={article.articleLength} /> : null;
  if (variant === "featured") return <article onClick={() => setPage({ name: "article", id: article.id })} className="anim-in" style={{ cursor: "pointer", borderRadius: "var(--radius-lg)", overflow: "hidden", background: "var(--bg-card)", border: "1px solid var(--border)", boxShadow: "var(--shadow-md)", transition: "var(--transition)", animationDelay: `${index * .1}s`, animationFillMode: "backwards", display: "grid", gridTemplateColumns: "1fr 1fr" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "none"; }}><div style={{ aspectRatio: "16/10", background: `url(${article.coverImage}) center/cover`, minHeight: 240 }} /><div style={{ padding: "32px 28px", display: "flex", flexDirection: "column", justifyContent: "center" }}>{cat && <Badge text={`${cat.icon} ${cat.name}`} color={cat.color} />}<h2 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "clamp(1.25rem,2vw,1.75rem)", lineHeight: 1.3, margin: "12px 0", color: "var(--text-primary)" }}>{article.title}</h2><p style={{ fontSize: 15, color: "var(--text-secondary)", lineHeight: 1.6, marginBottom: 16, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{article.excerpt}</p><div style={{ display: "flex", alignItems: "center", gap: 16, fontSize: 13, color: "var(--text-tertiary)", flexWrap: "wrap" }}><span style={{ display: "flex", alignItems: "center", gap: 4 }}><I n="clock" s={14} /> {article.readTime} min{lengthBadge}</span><span style={{ display: "flex", alignItems: "center", gap: 4 }}><I n="eye" s={14} /> {article.views?.toLocaleString()}</span><span>{formatDate(article.publishedAt)}</span></div></div></article>;
  return <article onClick={() => setPage({ name: "article", id: article.id })} className="anim-in" style={{ borderRadius: "var(--radius-lg)", overflow: "hidden", background: "var(--bg-card)", border: "1px solid var(--border)", boxShadow: "var(--shadow-sm)", cursor: "pointer", transition: "var(--transition)", animationDelay: `${index * .08}s`, animationFillMode: "backwards" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "var(--shadow-md)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "var(--shadow-sm)"; }}>{article.coverImage && <div style={{ aspectRatio: "16/9", background: `url(${article.coverImage}) center/cover`, position: "relative" }}>{cat && <span style={{ position: "absolute", top: 12, left: 12, padding: "4px 10px", borderRadius: 20, background: "rgba(0,0,0,.6)", backdropFilter: "blur(8px)", color: "#fff", fontSize: 11, fontWeight: 600, textTransform: "uppercase" }}>{cat.icon} {cat.name}</span>}{article.articleLength && article.articleLength !== "standard" && <span style={{ position: "absolute", top: 12, right: 12, padding: "4px 10px", borderRadius: 20, background: "rgba(0,0,0,.6)", backdropFilter: "blur(8px)", color: "#fff", fontSize: 10, fontWeight: 600 }}>{article.articleLength === "short" ? "📄 Short" : "📖 Long Read"}</span>}</div>}<div style={{ padding: "20px 20px 18px" }}><h3 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "1.125rem", lineHeight: 1.35, marginBottom: 8, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{article.title}</h3><p style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.55, marginBottom: 14, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{article.excerpt}</p><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 12, color: "var(--text-tertiary)" }}><div style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ display: "flex", alignItems: "center", gap: 3 }}><I n="clock" s={13} /> {article.readTime}m</span><span style={{ display: "flex", alignItems: "center", gap: 3 }}><I n="heart" s={13} /> {article.likes || 0}</span></div><span>{formatDate(article.publishedAt)}</span></div></div></article>;
};
        ...
        <nav style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {currentUser ? (
            <div ref={userMenuRef} style={{ position: "relative" }}>
              <button onClick={() => setUserMenu(v => !v)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", background: "var(--bg-card)", cursor: "pointer" }}>
              ...
              {userMenu && <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", boxShadow: "var(--shadow-lg)", minWidth: 200, padding: 4, zIndex: 200, animation: "slideDown .2s ease-out" }}>
...
const GlobalStyles = () => (
  <style>{`
    ...
    *{margin:0;padding:0;box-sizing:border-box} body,html{font-family:var(--font-body);background:var(--bg-primary);color:var(--text-primary);-webkit-font-smoothing:antialiased;line-height:1.6} ::selection{background:var(--accent);color:#fff} input,textarea,select,button{font-family:inherit;font-size:inherit} a{color:inherit;text-decoration:none}
    ...
    .article-body figure{margin:2em 0} .article-body figcaption{text-align:center} .article-body blockquote{margin:1.5em 0} .article-body img{max-width:100%;height:auto} .article-body hr{border:none;border-top:2px solid var(--border);margin:2em 0}
